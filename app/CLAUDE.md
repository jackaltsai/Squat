...
| M6 訓練統計 | ✅ 實機驗證通過 |
| M7 卡路里等 | ⏸ 已討論未開工 |
| M8 六動作架構 | 🚧 骨架完成，**六個動作中五個只有 UI、偵測未實作** |
...
| `claude/elderly-exercises-and-stats` | 六動作 + 統計（本分支） | M6 已驗證、M8 骨架完成 |
...
- [ ] 看格線（3 欄 × 2 列）：只有「深蹲」是亮的，其餘五個應標灰顯示「準備中」
- [ ] **六個圖示的比例需要檢查** —— 是在沒有視覺回饋的情況下寫出來的座標，很可能要調
- [ ] 確認深蹲流程完全沒壞（純重構，行為應與 M6 版本一致）
- [ ] 確認頂部顯示的是動作名稱（「深蹲」）而非「模式：入門 L1」

### 待決：M8 下一步先做哪個？
1. **坐站練習** —— 複用現有垂直位移狀態機，風險最低
2. **雙臂高舉 / 擴胸推掌** —— 驗證上肢關鍵點這條路是否可行
3. **踮腳尖** —— 位移最小、雜訊比最差，最晚做
...
**目標**：從「只有深蹲」擴展成長者取向的六個動作，並把 UI 改成圖示格線。

### 範圍修正（2026-09-23）
原規劃九個動作，涵蓋三種計量範式。**已縮減為六個，全部是次數型。**
移除的三個：側身伸展、單腳站立（持續型，計秒）、30秒坐站 STST（限時計次）。

隨之刪除的抽象：
- `ExerciseMeasurement` enum —— 六個動作全是次數型，留著一個只有單一值的 enum 是負擔不是彈性
- `targetSeconds` 欄位、`unit` 屬性（永遠是「下」）
- `CalibrationKind.NONE`（六個動作至少都需要站姿基準）

這個縮減同時砍掉了原本三項最麻煩的待辦：持續型計時器架構、`durationSeconds` 欄位
（Room v3 → v4）、統計頁「下」與「秒」分開加總。**資料庫維持 v3，不需要再遷移。**
...
### 校正方式仍需分開（`CalibrationKind`）
「站姿校正 + 兩下基準深蹲 → Duser → p = Dnow/Duser」只對深蹲家族成立：
手臂動作沒有「深度」可言，踮腳尖的位移也小到會淹沒在關鍵點雜訊裡。
