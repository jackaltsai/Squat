...
## 12. 實測資料揭露的兩個演算法問題（2026-09-10，列為 v4）

> 來源：2026-09-10 晚間匯出的 187 筆歷史紀錄 + 20 筆帶原始值的新紀錄。
> **兩項都不會 crash、不影響上架**，但**會影響論文的評估指標**，資料蒐集前必須處理。

### 🔴 問題 A：膝內夾判定測到的是雜訊

同一人、相隔 3～4 秒的連續 10 下，`kneeValgusRatio` 實測值：

```
+0.319(判定內夾)  -1.156  -1.228  -0.227  +0.149
+0.086  +0.354(判定內夾)  -1.027  -0.049  +0.462(判定內夾)
```

- **全距 1.690，判定門檻只有 0.15** —— 雜訊比訊號大 11 倍
- 20 筆中 14 筆為負值（負值本身正常，代表膝比踝寬＝正確姿勢；問題是**連續兩下跳動 1.5**，人的姿勢不可能這樣變）

**根本原因**（`KneeValgus.kt`）：

```kotlin
if (hipWidth / legLength < 0.2) return null       // 守門
return (ankleDistance - kneeDistance) / hipWidth  // 比值
```

`legLength = |ankleY - hipY|` 用**當幀**的值。蹲到最低點時髖部下降 → `legLength` 變小 → `hipWidth/legLength` 變大 → **守門在最需要它的時候反而最鬆**。不可靠的小 `hipWidth` 進了分母，比值就爆掉。
再加上前鏡頭透視：蹲深時膝蓋往鏡頭移動、影像上被放大，`kneeDistance` 虛增。2D 公式吃不下。

**對論文的影響**：膝內夾一致率（Accuracy / F1）是四項評估指標之一，目前數字不可信。

**v4 修法**：
1. 守門改用**站姿**的 legLength（站姿校正時已算過 `calibratedScale`），不隨深蹲變動
2. 取 BOTTOM **前後數幀的中位數**而非單幀 —— 單幀取樣本質上就不穩
3. 門檻仍須用人工標註影片重新校準（論文第 6 節）
