...
- [ ] **匯出全部歷史紀錄**：起始畫面（模式選擇下方）與結束摘要各有一個入口
  - [ ] 做完兩組、中間按過「重新開始」，再匯出全部 → 確認**兩組的資料都在**（這正是單場 CSV 會漏掉的情況）
  - [ ] 全新安裝、還沒做過任何一下時點下去 → 應跳「資料庫裡還沒有任何紀錄」而不是閃退
- [ ] 用 `python3 tools/analyze_sessions.py <資料夾>` 跑一次，確認能讀且會印出 Duser、且全部歷史紀錄有被切成多個場次
